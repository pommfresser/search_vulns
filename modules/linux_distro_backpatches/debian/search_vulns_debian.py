...
def postprocess_results(
    ...
    if vuln_db_cursor and "product_ids" in results and results["product_ids"].get("cpe", []):
        ...
        if latest_pkg_versions:
            latest = None
            query_version = CPEVersion(cpes[0].split(":")[5])
            for pkg, version_start, latest_version in latest_pkg_versions:
                latest_version = get_clean_version(latest_version)
                latest_version = CPEVersion(latest_version)
                version_start = CPEVersion(version_start)

                # skip entry with version_start > query_version
                if query_version and version_start > query_version:
                    continue

                # set latest version in first iteration
                if not latest:
                    latest = (pkg, version_start, latest_version)

                # update latest version if boundaries are tighter
                if version_start:
                    if version_start > latest[1] and query_version <= latest_version:
                        latest = (pkg, version_start, latest_version)
                # skip entry with no version start if entries with version start exists
                elif latest[1]:
                    continue
                else:
                    # compare latest versions
                    if query_version:
                        if query_version <= latest_version and latest[2] <= latest_version:
                            latest = (pkg, version_start, latest_version)
                    # find highest version if no version in query
                    elif latest[2] < latest_version:
                        latest = (pkg, version_start, latest_version)

            # craft version_status json
            pkg, version_start, latest_version = latest
